...
function ContactForm() {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12, textAlign: 'left' }}>
      ...
      {error && (
        <div style={{ background: 'rgba(255,80,80,0.08)', border: '1px solid rgba(255,80,80,0.3)', padding: '12px 16px', fontSize: 13, color: '#ff6b6b' }}>
          {error}
        </div>
      )}

      <button type="submit" disabled={submitting}
        style={{ marginTop: 8, background: submitting ? '#a8c430' : '#D4F53C', color: '#0C0C0E', border: 'none', padding: '18px 40px', fontSize: 14, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: submitting ? 'not-allowed' : 'pointer', transition: 'all 0.2s', fontFamily: "'Barlow Condensed', sans-serif", width: '100%' }}
        onMouseEnter={e => { if (!submitting) { e.currentTarget.style.opacity = '0.85'; e.currentTarget.style.transform = 'translateY(-2px)' } }}
        onMouseLeave={e => { e.currentTarget.style.opacity = '1'; e.currentTarget.style.transform = 'translateY(0)' }}>
        {submitting ? 'Sending...' : 'Join Symphony — Submit Application →'}
      </button>

      {/* Fallback email line */}
      <div style={{ marginTop: 20, textAlign: 'center', fontSize: 13, color: '#7A7A85' }}>
        Or email us directly at{' '}
        <a href="mailto:ridima@symphonyconsultancy.in"
          style={{ color: '#D4F53C', textDecoration: 'none', fontWeight: 600 }}
          onMouseEnter={e => (e.currentTarget.style.textDecoration = 'underline')}
          onMouseLeave={e => (e.currentTarget.style.textDecoration = 'none')}>
          ridima@symphonyconsultancy.in
        </a>
      </div>
    </form>
  )
